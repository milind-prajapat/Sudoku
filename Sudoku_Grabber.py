...
def Grab(Image):
    ...
    contours = [contour for contour in contours if cv2.contourArea(contour) >= Image.shape[0] * Image.shape[1] * 0.25]
    ...
    for box in contours:
        ...
        threshx = cv2.morphologyEx(threshx, cv2.MORPH_CLOSE, kernel, iterations = 1)
